...
def write_head(outfile,directory):
    f = open(os.path.join(rp.get_path("aist_scene_description"), "urdf/templates", 'aist_bin_template_front.xacro'),'r')
...
def write_tail(outfile,directory):
    f = open(os.path.join(rp.get_path("aist_scene_description"), "urdf/templates", 'aist_bin_template_tail.xacro'),'r')
...
def read_bin_layout(directory,set_origin):
    with open(os.path.join(rp.get_path("aist_scene_description"), "urdf/templates", 'placement_bin_layout.csv'), 'r') as f:
        reader = csv.reader(f)
        header = next(reader)
...
def read_bins_origin(directory):
    with open(os.path.join(rp.get_path("aist_scene_description"), "urdf/templates", 'bins_origin.csv'), 'r') as f:
        reader = csv.reader(f)
        header = next(reader)
        set_list = []
        for row in reader:
...
def read_bin_definition(directory):
    with open(os.path.join(rp.get_path("aist_scene_description"), "urdf/templates", 'placement_bin_definitions.csv'), 'r') as f:
        reader = csv.reader(f)
        header = next(reader)
        bin_definition=[]
        for row in reader:
...
def main():
    ...
    outfile = open(os.path.join(rp.get_path("aist_scene_description"), "urdf", 'kitting_bins.xacro'),'w+')
