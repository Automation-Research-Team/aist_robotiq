...
int main(int argc, char **argv) 
{
    try
    {
        ros::init(argc, argv, "realsense_camera_manager");
        ros::NodeHandle n;
        ...
        // Create librealsense context for managing devices and 
        ...
        {
            connected_devices.enable_device(dev);
            std::string serial_number = std::string(dev.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER));
            ROS_INFO("realsense device found. serial = %s", serial_number.c_str());
        }
        ...
        while (ros::ok())
        {
            ...
            {
                ROS_INFO("%s", no_camera_message.c_str());
                continue;
            }
            if (connected_devices.device_count() == 1)
            {
                ROS_INFO("%d cameras are connected. Please connect another camera.", total_number_of_streams);
            }
