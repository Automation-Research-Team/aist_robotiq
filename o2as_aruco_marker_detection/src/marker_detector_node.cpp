...
#include <o2as_aruco_marker_detection/DetectMarker.h>

namespace aruco_ros
{

void BREAKPOINT(std::string message)
{
  ROS_DEBUG_STREAM("BREAKPOINT: " << message);
  getchar();
}
...
class ArucoSimple
{
  ...
  void reconf_callback(ArucoThresholdConfig& config, uint32_t level);
  void image_callback(const image_p& image_msg);
  void cloud_callback(const cloud_p& cloud_msg);
  bool detect_marker_callback(
      o2as_aruco_marker_detection::DetectMarker::Request& req,
      o2as_aruco_marker_detection::DetectMarker::Response& res);
  void detect_marker(const image_t& image_msg, const cloud_t& cloud_msg, 
      o2as_aruco_marker_detection::DetectMarker::Response* res = NULL, bool useCloud = false);
  bool get_transform(const std::string& refFrame, 
      const std::string& childFrame, tf::StampedTransform& transform) const;
  tf::Transform get_marker_transform(
      const aruco::Marker& marker, const cloud_t& cloud_msg) const;
  template <class T> 
  cv::Vec<T, 3> view_vector(T u, T v) const;
...
private:
  ...
  ros::ServiceServer _detect_marker;
  dynamic_reconfigure::Server<ArucoThresholdConfig> _dyn_rec_server;
...
ArucoSimple::ArucoSimple()
  :_nh("~"),
   _it(_nh),
   _cam_info_sub(_nh.subscribe("/camera_info", 1, &ArucoSimple::cam_info_callback, this)),
   _image_sub(_it.subscribe("/image", 1, &ArucoSimple::image_callback, this)),
   _cloud_sub(_nh.subscribe("/pointcloud", 1, &ArucoSimple::cloud_callback, this)),
   ...
   _transform_pub(_nh.advertise<geometry_msgs::TransformStamped>("transform", 100)),
   _position_pub(_nh.advertise<geometry_msgs::Vector3Stamped>("position", 100)),
   ...
   _pixel_pub(_nh.advertise<geometry_msgs::PointStamped>("pixel", 10))
{
  ...
  _detect_marker = _nh.advertiseService("detect_marker", &ArucoSimple::detect_marker_callback, this);
  _dyn_rec_server.setCallback(boost::bind(&ArucoSimple::reconf_callback, this, _1, _2));
}
...
void ArucoSimple::cam_info_callback(const sensor_msgs::CameraInfo& msg)
{
  ...
  _rightToLeft.setOrigin(tf::Vector3(-msg.P[3]/msg.P[0], -msg.P[7]/msg.P[5], 0.0));
  _cam_info_sub.shutdown();
}

void ArucoSimple::reconf_callback(ArucoThresholdConfig& config, uint32_t level)
{
...
void ArucoSimple::image_callback(const image_p& image_msg)
{
...
void ArucoSimple::cloud_callback(const cloud_p& cloud_msg)
{
...
bool ArucoSimple::detect_marker_callback(
    o2as_aruco_marker_detection::DetectMarker::Request& req, 
    o2as_aruco_marker_detection::DetectMarker::Response& res) 
{
  try
  {
    res.success = false;
    _marker_id = req.marker_id;
    detect_marker(req.image, req.cloud, &res, req.use_cloud);
    return true;
  }
...
void ArucoSimple::detect_marker(
    const image_t& image_msg, const cloud_t& cloud_msg, 
    o2as_aruco_marker_detection::DetectMarker::Response* res, bool useCloud)
{
  if ((_image_pub.getNumSubscribers()	    == 0) &&
      ...
      res == NULL)
  {
    ROS_DEBUG_STREAM("No subscribers, not looking for aruco marker");
    return;
  }
  ...
  try
  {
    ...
    auto inImage = cv_bridge::toCvCopy(image_msg, sensor_msgs::image_encodings::RGB8)->image;
    ...
    for (const auto& marker : markers)
    {
      ...
      {
        // calculate transform from reference frame to the marker frame
        // tf::Transform	transform = get_marker_transform(marker, cloud_msg);
        // tf::Transform	transform = arucoMarker2Tf(marker);
        tf::Transform	transform;
        if (useCloud == true) {
          ROS_DEBUG_STREAM("calculate transform using point cloud.");
          transform = get_marker_transform(marker, cloud_msg);
        }
        else {
          ROS_DEBUG_STREAM("calculate transform without point cloud.");
          transform = arucoMarker2Tf(marker);
        }
        ...
        const tf::StampedTransform stampedTransform(transform, curr_stamp, _reference_frame, _marker_frame);
        ...
        // add detected markers to the response
        if (res) 
        {
          res->marker.pose = poseMsg;
          res->marker.id = marker.id;
          res->success = true;
        }
      }
    ...
    if (_image_pub.getNumSubscribers() > 0 || res != NULL)
    {
    ...
    if (_debug_pub.getNumSubscribers() > 0 || res != NULL)
    {
...
bool ArucoSimple::get_transform(const std::string& refFrame,
...
tf::Transform ArucoSimple::get_marker_transform(const aruco::Marker& marker,
          const cloud_t& cloud_msg) const
{
  ...
  const int	u0 = std::floor(std::min({marker[0].x, marker[1].x, marker[2].x, marker[3].x}));
  const int	v0 = std::floor(std::min({marker[0].y, marker[1].y, marker[2].y, marker[3].y}));
  const int	u1 = std::ceil( std::max({marker[0].x, marker[1].x, marker[2].x, marker[3].x}));
  const int	v1 = std::ceil( std::max({marker[0].y, marker[1].y, marker[2].y, marker[3].y}));
  ...
  const auto	c = (corners[2] + corners[3] - corners[0] - corners[1])
                + (corners[1] + corners[2] - corners[3] - corners[0]).cross(n);
  const auto	p = c / cv::norm(c);
  const auto	q = n.cross(p);

  // Compute marker centroid.
  const auto	centroid = 0.25*(corners[0] + corners[1] + corners[2] + corners[3]);
...
template <class T> 
cv::Vec<T, 3> ArucoSimple::view_vector(T u, T v) const
{
