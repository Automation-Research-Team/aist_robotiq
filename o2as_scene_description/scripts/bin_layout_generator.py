...
def write_bin_layout(outfile,directory,set_list):
    for s in set_list:
        ...
        for i in range(len(s.bins)):
            ...
            outfile.write(" <xacro:kitting_bin_"+str(s.bins[i].bin_type[3])+" binname=\""+str(s.bins[i].bin_name)+"\" parent=\"rack_bins_center\" z_origin_offset=\""+str(s.bins[i].definition.z_origin_offset)+"\">\n ")
