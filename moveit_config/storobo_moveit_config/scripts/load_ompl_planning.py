...
def main(args):
    ...
    f = open(args.ompl_planning_yaml, 'r')
    planner_configs = yaml.load(f)
    f.close()

    output = {}
    output["planner_configs"] = planner_configs

    for group, attributes in groups.items():
        output[group + "/" + "planner_configs"] = planner_configs.keys()
        output[group + "/" + 'default_planner_config'] = 'RRTConnectkConfigDefault'
...
def parse_arguments(args):
    ...
    parser.add_argument("--ompl_planning_yaml", type=str, default="")
